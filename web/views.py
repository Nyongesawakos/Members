...
def index(request, pk):
    ...
    context ={'index': index, 'payment':payment,'R_messages': R_messages, 'adm':adm, 'total_amount': total_amount}
...
#def pay(request, pk):
    #pays=update.objects.get(id=pk) 
    #payment=update.objects.all()
    #context ={'pays': pays , 'payment': payment}
    #return render(request, 'web/pay.html', context)
#def check(request):
    #payment=update.objects.all() 
    #detail=room.objects.all()
    #context ={'payment': payment, 'detail':detail}
    #return render(request, 'web/check.html', context)
